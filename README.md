...
Switch lets Claude Code checkpoint its task, send `/clear` through **Herdr**, and
resume in a fresh session with a verified acknowledgment. It handles the reset
itself, beyond saving and reloading a handoff; **Herdr is required**.

`checkpoint → request → end turn → /clear → bootstrap → acknowledge → next action`

The optional [context-limit companion](docs/auto.md) requests preparation at a
limit such as `250k` tokens or `25%` of the reported window. This is a soft
threshold: completing the current response and active work can overshoot it.
Switch runs one bounded helper per request, with no resident service or pip dependencies.

## Requirements and validation status

- Python **3.9+**, standard library only. Tested on macOS with Python 3.9.6 and 3.14.7.
- [Herdr](https://github.com/herdrdev/herdr/tree/v0.7.5) **0.7.5** and Claude Code
  **2.1.278** are the earliest versions actually tested. Older versions are
  unverified; compatibility with newer terminal layouts is not guaranteed.
- Claude must run inside a Herdr pane, with Herdr's Claude integration reporting
  its current session. Follow [Herdr's setup](https://herdr.dev/docs/integrations).
  Switch needs the `herdr` CLI on PATH and access to its local socket.
- A local POSIX filesystem supporting advisory locks and directory fsync.
  Windows and other platforms have not been validated.

**Proven:** v0.2 reset, bootstrap and acknowledgment with a manual trigger in
Claude auto permission mode (Fable 5.1 dogfood, September 20, 2026).
Threshold-triggered end-to-end validation of v0.2 in auto permission mode is
**pending**.
A Sonnet 5 auto-mode run without request/ack allow rules was denied as
`Tmux Self Drive` before any clear. Auto mode alone does not establish permission
to reset. The successful run did not capture launch-time allow rules, so it does
not establish which rule or classifier decision allowed it. See [validation](docs/validation.md).

The companion retains verbatim prompt inputs in private local files, up to
512 KiB per session, throughout its installed scope, including nested sessions.
There is no automatic expiry. One reset attempt is allowed per session:
even `not_cleared` retains its claim and requires deliberate manual recovery.
...
For user scope, status, uninstall and retention details, see [automatic setup](docs/auto.md).
...
4. After request reports `scheduled`, end the turn immediately. Start no more
...
Claims persist per old session, including failed requests, to prevent destructive
replay. There is deliberately no automatic retry or re-clear command. After a
failure, inspect `request.json`, `ack.json`, current session and hook evidence.
If clearing succeeded and the bootstrap did not run, preserve any draft and
manually submit saved `bootstrap.txt` once. If it already ran, continue there.
For an unconfirmed clear, reconcile the old/new session first: a timed-out send
may still arrive. Never use request again as a recovery shortcut.
...
preparation. See [the design](docs/design.md) and [validation](docs/validation.md).
